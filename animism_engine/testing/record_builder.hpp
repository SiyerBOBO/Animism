...
namespace animism_engine { namespace testing {
    ...
    public:
        record_builder(
            const animism_engine::guid &providerId,
            size_t id,
            size_t version,
            size_t opcode = 0,
            size_t level = 0,
         ...
         * </example>
         */
       animism_engine::testing::synth_record pack() const;
         ...
         * </example>
         */
        animism_engine::testing::synth_record pack_incomplete() const;
    ...
    private:
        const animism_engine::guid &providerId_;
        const size_t id_;
...
#pragma warning(pop)
        ...
        return animism_engine::testing::synth_record(record, results.first, extended_data_buffer.first);
    }
    ...
    record_builder::pack_impl(const EVENT_RECORD &record) const
    {
        ...
        animism_engine::schema_locator schema_locator;
        animism_engine::schema event_schema(record, schema_locator);
        animism_engine::parser event_parser(event_schema);
            ...
            if (found_prop != properties_.end()) {
                ...
                if (found_prop->type() != prop.type()) {
                    ...
                    auto msg = std::string(
                        ...
                        " Expected: " + animism_engine::in_type_to_string(prop.type()) +
                        " Received: " + animism_engine::in_type_to_string(found_prop->type()));
