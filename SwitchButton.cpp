...
void SwitchButton::setChecked(bool checked)
{
	...
	if (this->checked != checked)
		QTimer::singleShot(step ? 0 : 10, this, [=]() {mousePressEvent(NULL); });
}

void SwitchButton::setCheckedWithoutSignal(bool checked)
{
	//如果刚刚初始化完成的属性改变则延时处理

	if (this->checked != checked) {
		this->checked = checked;
		QTimer::singleShot(step ? 0 : 10, this, [=]() {this->procAnimate(); });
	}
}
