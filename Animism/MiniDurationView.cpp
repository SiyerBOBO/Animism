...
MiniDurationView::MiniDurationView(QWidget *parent)
	: QWidget(parent)
{
	this->initUi();
}

void MiniDurationView::initUi()
{
	ui.setupUi(this);
...
bool MiniDurationView::event(QEvent* e)
{
...
void MiniDurationView::loadData()
{
...
void MiniDurationView::reset()
{
	if (this->_ptr_watcher_event)
		this->_ptr_watcher_event->deleteLater();
	if (this->_ptr_watcher_count)
		this->_ptr_watcher_count->deleteLater();
	...
	this->_ptr_watcher_count = new QFutureWatcher<QMap<QString, qulonglong>>(_spointer_instance.get());
	QObject::connect(this->_ptr_watcher_count, &QFutureWatcher<QList<EventInfo>>::finished, this, [=]() {
		auto _data = this->_ptr_watcher_count->result();
		qulonglong _value = 0;
		for (auto item : _data)
			_value += item;
		this->ui.label_data_count->setText(tr("共计%1条事件").arg(_value));
		});
	this->ui.label_data_count->setText(tr("正在查询..."));
	this->_ptr_watcher_count->setFuture(Reader.opcode_count(this->_str_task_name, this->_timestamp_from, this->_timestamp_to));
	...
	this->ui.label_data_duration->setText(tr("%1至%2")
		.arg(this->_timestamp_from.toString(Qt::DateFormat::ISODateWithMs))
		.arg(this->_timestamp_to.toString(Qt::DateFormat::ISODateWithMs))
	);
	this->loadData();
}

std::unique_ptr<MiniDurationView> MiniDurationView::_spointer_instance;
MiniDurationView* MiniDurationView::Instance(const QString& task_name, const QDateTime& from, const QDateTime& to)
{
	static std::once_flag oc;
	std::call_once(oc, [&] {  _spointer_instance.reset(new MiniDurationView()); });
	_spointer_instance->_str_task_name = task_name;
	_spointer_instance->_timestamp_from = from;
	_spointer_instance->_timestamp_to = to;
	_spointer_instance->reset();
	return _spointer_instance.get();
}

MiniDurationView::~MiniDurationView()
{
}
