    ...
    inline void error_check_common_conditions(ULONG status)
    {
        ...
        switch (status) {
            case ERROR_ALREADY_EXISTS:
                throw animism_engine::trace_already_registered();
            case ERROR_INVALID_PARAMETER:
                throw animism_engine::invalid_parameter();
            case ERROR_ACCESS_DENIED:
                throw animism_engine::need_to_be_admin_failure();
            case ERROR_NOT_FOUND:
                throw animism_engine::could_not_find_schema();
            case ERROR_NO_SYSTEM_RESOURCES:
                throw animism_engine::no_trace_sessions_remaining();
