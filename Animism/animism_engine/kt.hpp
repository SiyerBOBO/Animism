...
namespace animism_engine { namespace details {
    ...
    struct kt {
        ...
        typedef animism_engine::kernel_provider provider_type;
        ...
        static const unsigned long construct_enable_flags(
            const animism_engine::trace<animism_engine::details::kt> &trace);
        ...
        const std::wstring &name_hint)
    {
        if (IsWindows8OrGreater()) {
            return animism_engine::details::ut::enforce_name_policy(name_hint);
        }
    ...
    inline animism_engine::guid kt::get_trace_guid()
    {
        if (IsWindows8OrGreater()) {
            return animism_engine::guid::random_guid();
        }

        return animism_engine::guid(SystemTraceControlGuid);
    }
