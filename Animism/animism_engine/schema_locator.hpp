...
namespace std {
     ...
     */
    template<>
    struct std::hash<animism_engine::schema_key>
    {
        size_t operator()(const animism_engine::schema_key &key) const
        {
            // Shift-Add-XOR hash - good enough for the small sets we deal with
            size_t h = 2166136261;

            h ^= (h << 5) + (h >> 2) + std::hash<animism_engine::guid>()(key.provider);
            h ^= (h << 5) + (h >> 2) + key.id;
...
namespace animism_engine {
     ...
     * when animism_engine is compiled into a managed assembly.
     * </summary>
     */
