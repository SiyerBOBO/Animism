...
namespace std
{
    inline std::wstring to_wstring(const animism_engine::guid& guid)
    {
        wchar_t* guidString;
        ...
        std::unique_ptr<wchar_t, animism_engine::CoTaskMemDeleter> managed(guidString);

        return { managed.get() };
    }

    template<>
    struct std::hash<animism_engine::guid>
    {
        size_t operator()(const animism_engine::guid& guid) const
        {
